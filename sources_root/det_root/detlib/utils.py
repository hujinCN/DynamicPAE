...
from det_root.utils.parser import logger_msg
...
def init_detector(detector_name: str, cfg: object, device: torch.device =torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
    ...
    elif detector_name == "yolov5m":
        from detlib.HHDet import HHYolov5
        detector = HHYolov5(name=detector_name, cfg=cfg, device=device)
        if cfg.PERTURB.GATE == 'shakedrop':
            model_config = 'HHDet/yolov5/yolov5/models/yolov5m-shakedrop.yaml'
        elif cfg.PERTURB.GATE == 'ghostshake':
            model_config = 'HHDet/yolov5/yolov5/models/yolov5m-ghostshake.yaml'
        elif model_config is None:
            model_config = 'HHDet/yolov5/yolov5/models/yolov5m.yaml'
        print(DET_LIB)
        print("!!!!!")
        detector.load(
            model_weights=os.path.join(DET_LIB, 'HHDet/yolov5/yolov5/weight/yolov5m.pt'),
            model_config=os.path.join(DET_LIB, model_config)
        )
