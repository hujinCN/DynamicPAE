...
def analyze_results_cat_bbox(cocoGt,
                    cocoDt,
                    # res_types,
                    out_dir,
                    analyze_cat_id = [1],
    ...
    cocoEval.params.imgIds = imgIds
    cocoEval.params.iouThrs = [0.75, 0.5, 0.1]
    cocoEval.params.maxDets = [100, 50, 25]
    if areas:
        cocoEval.params.areaRng = [[0**2, areas[2]], [0**2, areas[0]],
                                   [areas[0], areas[1]],
                                   [areas[1], areas[2]]]
    ...
    catIds = cocoGt.getCatIds()
    recThrs = cocoEval.params.recThrs
    analyze_results_cat_bbox = []
    for k, catId in enumerate(analyze_cat_id):
        analyze_results_cat_bbox.append(analyze_individual_category(k, cocoDt, cocoGt, catId, iou_type, areas))
    # with Pool(processes=min(48, len(analyze_cat_id))) as pool:
    #     args = [(k, cocoDt, cocoGt, catId, iou_type, areas)
    #             for k, catId in enumerate(analyze_cat_id)]
    #     analyze_results_cat_bbox = pool.starmap(analyze_individual_category, args)
    for k, catId in enumerate(analyze_cat_id):
        nm = cocoGt.loadCats(catId)[0]
        print(f'--------------saving {k + 1}-{nm["name"]}---------------')
        analyze_results_cat_bbox = analyze_results_cat_bbox[k]
        assert k == analyze_results_cat_bbox[0]
        ps_supercategory = analyze_results_cat_bbox[1]['ps_supercategory']
        ps_allcategory = analyze_results_cat_bbox[1]['ps_allcategory']
        # compute precision but ignore superclass confusion
        ps[3, :, k, :, :] = ps_supercategory
        # compute precision but ignore any class confusion
        ps[4, :, k, :, :] = ps_allcategory
...
def main():
    ...
    parser.add_argument('--result', help='result file (json format) path',
                        default="result.json")
    parser.add_argument('--out_dir', help='dir to save analyze result images',  default="./coco_test")
    parser.add_argument(
        '--ann',
...
if __name__ == '__main__':
    cocoGt = COCO("/home/hujin/dataset/coco/annotations/instances_val2017.json")
    cocoDt = cocoGt.loadRes("/home/hujin/dynamic_attack/logs/dynamic_example/yolo/detection_result_clean.txt")
    analyze_results_cat_bbox(cocoGt, cocoDt, "/home/hujin/dynamic_attack/logs/dynamic_example/yolo/999_baseline/")
    # main()
